...
/// A [CustomChatView] displays a scrollable list of chat messages associated with the
...
/// The state of the [CustomChatView].
class CustomChatViewState extends State<CustomChatView>
  ...
  void initState() {
    ...
    //   widget.controller.startReadingMode();
    // });
  ...
  void dispose() {
    ...
    // TODO: вернуть
    // widget.controller.endReadingMode();
  ...
  Widget _chatItemBuilder(BuildContext context, int item) {
    ...
    final message = widget.controller.messages[item];
    ChatMessage? previous;
    if (item > 0) {
      previous = widget.controller.messages[item - 1];
    }

    return widget.tileBuilder(context, message, previous);
  }
}
