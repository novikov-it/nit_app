...
import 'package:nit_app/nit_app.dart' as nit;
import 'package:nit_app/src/nit_alerts/nit_alerts.dart';
...
/// The [NitSessionManager] keeps track of and manages the signed-in state of the
...
class NitSessionManager with ChangeNotifier {
  static NitSessionManager? _instance;
  ...
  static Future<NitSessionManager> get instance async {
  ...
  Future<bool> signOut({
    ...
    try {
      // await caller.status.signOut(revokeAllTokens: revokeAllTokens);
      await nit.nitToolsCaller!.nitAuth.signOutUser(
  ...
  Future<bool> refreshSession() async {
    try {
      ...
      await NitAlerts.sendAlert(
        message: '⚠️ NitSessionManager: refreshSession error: $e',
      );
      return false;
    }
  }
